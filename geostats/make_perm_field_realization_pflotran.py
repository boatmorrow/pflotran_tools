...
#model mesh is 500m by 500m with 500 x 500 cells so ranges need to be scaled 
variogram2 = CovarianceModel(type=covariance.spherical, ranges = (20, 20, 1), sill = 1, angles = (0, 0, 0))
sgs_params = {"cov_model": variogram2, "radiuses": (40, 40, 1), "max_neighbours": 20 }
...
#pdb.set_trace();
t = logspace(floor(log10(a.min())),ceil(log10(a.max())),num=5);
#ffmt = matplotlib.ticker.ScalarFormatter(useOffset=False);
#ffmt.set_scientific(True);
ffmt = matplotlib.ticker.LogFormatterMathtext(labelOnlyBase=True);
#bar = colorbar(ticks=t,format='%.2e',orientation='vertical');
bar = colorbar(format=ffmt,orientation='vertical');
